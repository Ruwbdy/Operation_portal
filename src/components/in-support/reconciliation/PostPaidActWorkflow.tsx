...
export default function PostPaidActWorkflow({ onBack, onSuccess, onError }: PostPaidActWorkflowProps) {
  ...
  const runJob1 = async () => {
    ...
    const downloads = res.blob ? [{ name: res.filename || 'pre_to_post_result.csv', blob: res.blob }] : [];
    setJob1State({
      status: 'done',
  ...
  const runJob2 = async () => {
    ...
    const downloads = res.blob ? [{ name: res.filename || 'credit_limits_result.csv', blob: res.blob }] : [];
    setJob2State({
      status: 'done',
