...
type JobType = 'INIT_SIM_REG' | 'GET_SIM_REG_IMSI' | 'COMPLETE_SIM_REG';
...
const JOBS: Job[] = [
  {
    id: 'INIT_SIM_REG',
    label: 'Initiate SIM Registration',
    description: 'Start bulk SIM registration process for new subscribers',
    filesNeeded: 1
  },
  {
    id: 'GET_SIM_REG_IMSI',
    label: 'Get SIM Registration IMSI',
    description: 'Retrieve IMSI data for pending SIM registrations',
    filesNeeded: 1
  },
  {
    id: 'COMPLETE_SIM_REG',
    label: 'Complete SIM Registration',
    description: 'Finalize bulk SIM registration operations',
    filesNeeded: 1
  }
];

export default function DCLM() {
        ...
        <header className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-16">
          <div className="space-y-3 min-w-0">
            ...
            <p className="text-sm font-bold text-gray-500 leading-relaxed max-w-2xl">
              Batch automation for SIM registration workflows including initiation, IMSI retrieval, and bulk completion processes.
            </p>
          </div>
        </header>

        {/* Job Selection or Active Job */}
        {activeJob === null ? (
